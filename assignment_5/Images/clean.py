...
def get_neighbour_labels(image_array_label, x, y, num_neighbours):
    label_count = {}
    if num_neighbours == 4:
...
def run_clean(image_array_label, num_neighbours, threshold, iterations):
    ...
    for i in range(iterations):
        for x in range(image_array_label.shape[0]):
            for y in range(image_array_label.shape[1]):
                ...
                label_count = get_neighbour_labels(image_array_label, x, y, num_neighbours)
